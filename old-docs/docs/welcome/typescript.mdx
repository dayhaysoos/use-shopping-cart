...
This library now works with TypeScript! You can use `use-shopping-cart` just the same as before but now with types. Take for example these two files from our very own [`typescript-usage` example workspace](https://github.com/dayhaysoos/use-shopping-cart/tree/legacy-examples/examples/typescript-usage).
...
The above file didn't really use types yet but it is type-safe. Let's get into some more TypeScript specific use cases.
